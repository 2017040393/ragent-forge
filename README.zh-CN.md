...
v0.3 计划把 workspace-local project memory 与 retrieval quality maturation 结合：
引入 typed document/memory sources、inspectable retrieval stages、optional query
rewriting/expansion 和 reranking，以及 source-aware traces 和 evaluation。v0.4
在这条 pipeline 上构建受控的 multi-step retrieval 和 agent workflows；v0.5
增加 retrieval quality 与 answer quality 的本地 comparison views。这些是未来
方向，不是当前 v0.2 功能。
