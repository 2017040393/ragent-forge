...
v0.3 是 retrieval quality and efficiency engineering 阶段。它会在选择技术方案
之前冻结 benchmark 和相对 baseline 的发布门槛，然后量化提升 candidate recall、
final precision、retrieval latency 和 context cost。Workspace-local project
memory 会与 document evidence、user notes、session memory 共用一个 retrieval
入口；每条记录仍保留 source type、provenance 和 lifecycle，并使用同一套
stage-aware traces 和 evaluation 进行测量。v0.4
在这条 pipeline 上构建受控的 multi-step retrieval 和 agent workflows；v0.5
增加 retrieval quality 与 answer quality 的本地 comparison views。v0.3 是当前
开发阶段；v0.4 和 v0.5 仍是未来方向。
