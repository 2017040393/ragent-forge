...
普通输入表示 Ask。如果 composer 中包含普通文本，shell 会使用当前 retrieval
settings 运行现有 Ask flow。默认 retrieval mode 是 `hybrid`。
...
- `/export markdown|json` 将当前 session 写入 `.ragent/sessions/exports/`。
- `/branch`、`/rerun`、`/continue-sources`、`/title [auto|text]` 和
  `/turn <id|number|next|prev|first|last>` 作用于 selected answer。
- `/help` 显示可用 commands。
- `/clear` 清空 transcript。
- `/exit` 和 `/quit` 退出 TUI。
...
Aliases 应保持稀疏且可预测。Parser 不应增加 fuzzy matching。
...
composer 和 transcript 响应。配置的 generation provider 支持 streaming 时，Ask
会把 assistant deltas 流式写入 transcript。

Worker completion 应在 UI thread 上 append transcript messages 并更新 shell
state。Worker failures 应产生友好的 error messages、保存 failed assistant turn，
并且绝不显示 stack traces 或 API keys。Shell Search 和 Shell Ask 是基于当前本地
...
当前 TUI 会在 `.ragent/sessions/` 下持久化本地 conversation sessions：

- `latest.json` 指向下次 TUI 启动时恢复的 session。
- `index.json` 存放 session picker 使用的紧凑 session summaries。
- `session-<id>.json` 存放 turns、assistant sources、run metadata、title、
  pin/star state 和可选 branch metadata。
- `exports/` 存放 `/export` 创建的 Markdown 或 JSON exports。
...
source inspection、transcript polish、saved sessions 内搜索、可选的 richer status
panels，以及更好的 review/export workflows。
...
- 不打开本地文件。
- 无完整 source-management table UI。
- 无 agent tool loop。
