...
指出 inline command suggestions、background Shell Search、search results 后打开的
source picker、干净 chat transcript 中的轻量 answer badges，以及 selected-source
Inspector。
...
说明 source navigation 仍是 command-first；当前 `main` 也提供聚焦的
source/session pickers。Source picker rows 会显示紧凑 location、retrieval method、
score 和 chunk id；`/source <rank|next|prev>` 仍保留给精确 command input。
...
没有 `q` 这样的全局单键快捷键。
如果请求正在运行，composer 仍可输入 draft，并显示 `1 draft queued`；当前请求完成后
显示 `1 draft ready`。
