...
`src/ragent_forge/infrastructure/` 中的 adapters 由 composition 组装。为了兼容
旧调用方，`app/workspace.py` 和 `app/storage.py` 仍保留 compatibility import
facades；新的 presentation code 通过 composition 使用 infrastructure adapters。
...
此外，ingest 会创建 workspace snapshot。`.ragent/snapshot.json` manifest 记录当前
snapshot id、source path、创建时间和 chunk count。Chunks、ingest summaries、traces
以及 vector-index records/manifests 都引用同一个 id。读取器会拒绝混合代次，也会拒绝
在 manifest 尚未提交时读取带 snapshot 标记的 artifacts。这样即使底层仍是普通
JSON/JSONL 文件，workspace 也有清晰的一致代次边界。
...
当前架构已经提供 typed retrieval contracts、显式 single-pass stages、snapshot
consistency boundary 和 adapter seams，作为 v0.3 的基础。Reranker 在配置具体实现
之前会有意保持为 skipped stage。v0.3 可以在同一条 pipeline 上增加 typed document
和 project-memory sources；v0.4 再基于这些显式 stages 构建受控的 query refinement
和 iterative retrieval；v0.5 增加 retrieval quality 与 answer quality 的本地
comparison views。
