...
def test_ingests_markdown_and_txt_files_recursively(tmp_path: Path) -> None:
    ...
    (knowledge_dir / "ignore.pdf").write_text("ignored", encoding="utf-8")
    ...
    assert result.skipped_files == [str((knowledge_dir / "ignore.pdf").resolve())]
...
def test_ingest_rejects_path_with_no_supported_files(tmp_path: Path) -> None:
    (tmp_path / "only.pdf").write_text("not supported", encoding="utf-8")
